...
- Final safe source commit used for package lock and final validator evidence: the repository `HEAD` resolved by `commit=$(git rev-parse HEAD)` in the final clean-run lock-regeneration command below.
- Evidence provenance: the required final clean-run block refreshes `validator/artifacts/libvips-safe-port-lock.json` and `validator/artifacts/libvips-safe-final/proof/libvips-safe-validation-proof.json`; after that rerun, their commit fields must match the same `git rev-parse HEAD` value with release tag `build-${commit:0:12}`.
...
- Lock/proof provenance verification: after the final clean-run block, `validator/artifacts/libvips-safe-port-lock.json` `.libraries[0].commit` and `validator/artifacts/libvips-safe-final/proof/libvips-safe-validation-proof.json` `.libraries[0].port_commit` match `git rev-parse HEAD`.
