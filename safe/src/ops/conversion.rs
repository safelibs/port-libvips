...
unsafe fn op_gravity(object: *mut VipsObject) -> Result<(), ()> {
    ...
    let (width_i, height_i) = (
        isize::try_from(width).map_err(|_| ())?,
        isize::try_from(height).map_err(|_| ())?,
    );
    let (input_width, input_height) = (
        isize::try_from(input.spec.width).map_err(|_| ())?,
        isize::try_from(input.spec.height).map_err(|_| ())?,
    );
    let x_mid = (width_i - input_width) / 2;
    let y_mid = (height_i - input_height) / 2;
    let (x, y) = match direction {
        VIPS_COMPASS_DIRECTION_CENTRE => (x_mid, y_mid),
        VIPS_COMPASS_DIRECTION_NORTH => (x_mid, 0),
        VIPS_COMPASS_DIRECTION_EAST => (width_i - input_width, y_mid),
        VIPS_COMPASS_DIRECTION_SOUTH => (x_mid, height_i - input_height),
        VIPS_COMPASS_DIRECTION_WEST => (0, y_mid),
        VIPS_COMPASS_DIRECTION_NORTH_EAST => (width_i - input_width, 0),
        VIPS_COMPASS_DIRECTION_SOUTH_EAST => (width_i - input_width, height_i - input_height),
        VIPS_COMPASS_DIRECTION_SOUTH_WEST => (0, height_i - input_height),
